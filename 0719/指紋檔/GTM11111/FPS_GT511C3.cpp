...
byte* Command_Packet::GetPacketBytes()
{
	...
	packetbytes[0] = COMMAND_START_CODE_1; //packetbytes[0]= 55
	packetbytes[1] = COMMAND_START_CODE_2; //packetbytes[1]= AA
 	packetbytes[2] = COMMAND_DEVICE_ID_1; //packetbytes[2]= 
...
void Command_Packet::ParameterFromInt(int i)// Command_Packet.ParameterFromlnt(i)
{
...
int FPS_GT511C3::Enroll1()
{
	...
	Command_Packet* cp = new Command_Packet();
	cp->Command = Command_Packet::Commands::Enroll1; //cp=Command_Packet.Commands.Enrolll
	byte* packetbytes = cp->GetPacketBytes();
	delete cp;
	SendCommand(packetbytes, 12);
	delete packetbytes;
...
#endif  //__GNUC__
// Sends the command to the software serial channel將命令發送到軟件串行通道
...
// Gets the response to the command from the software serial channel (and waits for it)從軟件串行通道獲取對命令的響應（並等待它）
...
// sends the bye aray to the serial debugger in our hex format EX: "00 AF FF 10 00 13"將再見aray發送到我們的十六進制格式的串行調試器EX：“00 AF FF 10 00 13”
...
// sends a byte to the serial debugger in the hex format we want EX "0F"以十六進制格式向串行調試器發送一個字MN節，我們想要EX“0F”
