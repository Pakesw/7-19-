	...
	public:
		class Commands
		{
			public:
				enum Commands_Enum
				{
					NotSet				= 0x00,		// Default value for enum. Scanner will return error if sent this.  枚舉的默認值。 如果發送此信息，掃描儀將返回錯誤
					Open				= 0x01,		// Open Initialization   打開初始化
					Close				= 0x02,		// Close Termination     關閉終止
					UsbInternalCheck	= 0x03,		// UsbInternalCheck Check if the connected USB device is valid     UsbInternalCheck檢查連接的USB設備是否有效
					ChangeEBaudRate		= 0x04,		// ChangeBaudrate Change UART baud rate    ChangeBaudrate更改UART波特率
					SetIAPMode			= 0x05,		// SetIAPMode Enter IAP Mode In this mode, FW Upgrade is available   SetIAPMode進入IAP模式在此模式下，可以使用FW升級
					CmosLed				= 0x12,		// CmosLed Control CMOS LED  CmosLed控制CMOS LED
					GetEnrollCount		= 0x20,		// Get enrolled fingerprint count   獲取登記指紋計數
					CheckEnrolled		= 0x21,		// Check whether the specified ID is already enrolled 檢查指定的ID是否已註冊
					EnrollStart			= 0x22,		// Start an enrollment  開始註冊
					Enroll1				= 0x23,		// Make 1st template for an enrollment 為註冊製作第一個模板
					Enroll2				= 0x24,		// Make 2nd template for an enrollment 為註冊製作第二個模板
					Enroll3				= 0x25,		// Make 3rd template for an enrollment, merge three templates into one template, save merged template to the database 為註冊創建第3個模板，將三個模板合併為一個模板，將合併模板保存到數據庫
					IsPressFinger		= 0x26,		// Check if a finger is placed on the sensor  檢查手指是否放在傳感器上
					DeleteID			= 0x40,		// Delete the fingerprint with the specified ID  刪除具有指定ID的指紋
					DeleteAll			= 0x41,		// Delete all fingerprints from the database  刪除數據庫中的所有指紋
					Verify1_1			= 0x50,		// Verification of the capture fingerprint image with the specified ID  使用指定的ID驗證捕獲指紋圖像
					Identify1_N			= 0x51,		// Identification of the capture fingerprint image with the database  使用數據庫識別捕獲指紋圖像
					VerifyTemplate1_1	= 0x52,		// Verification of a fingerprint template with the specified ID   驗證具有指定ID的指紋模板
					IdentifyTemplate1_N	= 0x53,		// Identification of a fingerprint template with the database   使用數據庫識別指紋模板
					CaptureFinger		= 0x60,		// Capture a fingerprint image(256x256) from the sensor  從傳感器捕獲指紋圖像（256x256）
					MakeTemplate		= 0x61,		// Make template for transmission   製作傳輸模板
					GetImage			= 0x62,		// Download the captured fingerprint image(256x256)   下載捕獲的指紋圖像（256x256）
					GetRawImage			= 0x63,		// Capture & Download raw fingerprint image(320x240)   捕獲和下載原始指紋圖像（320x240）
					GetTemplate			= 0x70,		// Download the template of the specified ID   下載指定ID的模板
					SetTemplate			= 0x71,		// Upload the template of the specified ID   上傳指定ID的模板
					GetDatabaseStart	= 0x72,		// Start database download, obsolete  啟動數據庫下載，已過時
					GetDatabaseEnd		= 0x73,		// End database download, obsolete    結束數據庫下載，已過時
					UpgradeFirmware		= 0x80,		// Not supported  不支持
					UpgradeISOCDImage	= 0x81,		// Not supported  不支持
					Ack					= 0x30,		// Acknowledge.  確認。
					Nack				= 0x31		// Non-acknowledge   不承認
			};
		};

		Commands::Commands_Enum Command;
		byte Parameter[4];								// Parameter 4 bytes, changes meaning depending on command		參數4 bytes，根據命令改變含義					
		byte* GetPacketBytes();							// returns the bytes to be transmitted  返回要傳輸的字節
		void ParameterFromInt(int i);

		Command_Packet();

	private: 
		static const byte COMMAND_START_CODE_1 = 0x55;	// Static byte to mark the beginning of a command packet	-	never changes  用於標記命令包開頭的靜態字節 - 永遠不會更改
		static const byte COMMAND_START_CODE_2 = 0xAA;	// Static byte to mark the beginning of a command packet	-	never changes  用於標記命令包開頭的靜態字節 - 永遠不會更改
		static const byte COMMAND_DEVICE_ID_1 = 0x01;	// Device ID Byte 1 (lesser byte)							-	theoretically never changes  設備ID字節1（較小的字節） - 理論上永遠不會改變
		static const byte COMMAND_DEVICE_ID_2 = 0x00;	// Device ID Byte 2 (greater byte)						-	theoretically never changes  設備ID字節2（更大的字節） - 理論上永遠不會改變
		byte command[2];								// Command 2 bytes

		word _CalculateChecksum();						// Checksum is calculated using byte addition   驗證和使用字節添加計算
...
class Response_Packet
{
	public:
		...
		ErrorCodes::Errors_Enum Error; //ErrorCodes.Errors_Enum Error;
		byte RawBytes[12];
		byte ParameterBytes[4]; //能存放4Bytes
